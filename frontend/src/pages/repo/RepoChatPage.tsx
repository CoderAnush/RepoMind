...
import {
  ...
  ChevronDown, ChevronUp, Copy, Check, FileCode, AlertCircle, ArrowUpRight
} from 'lucide-react';
...
export default function RepoChatPage() {
  ...
  const handleSendMessage = async (msgText: string) => {
    ...
    try {
      // Send message to RAG pipeline. Support both session_id (backend schema) and conversation_id (frontend ts types)
      const res = await chatService.send({
        repository_id: id!,
        message: msgText,
        conversation_id: sessionId,
        ...({ session_id: sessionId } as any) // bypass strict ts constraint for backend compatibility
      } as any);

      // Save session id if not set
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-zinc-950">
        ...
        {messages.length === 0 ? (
          <div className="max-w-xl mx-auto py-16 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xs text-zinc-500 mt-2 max-w-sm leading-relaxed">
              Ask questions about classes, execution paths, dependencies, and configuration. The assistant cites original source codes.
            </p>
                ...
                <div key={index} className={`flex gap-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[85%] rounded-xl p-4 border ${
                    ...
                    {!isUser && msg.references && msg.references.length > 0 && (
                      <div className="border-t border-zinc-800/60 pt-3.5 mt-3.5">
                        <button
            ...
            {loading && (
              <div className="flex gap-4">
                ...
                <div className="bg-zinc-900/30 border border-zinc-850 p-4 rounded-xl max-w-[85%] flex items-center gap-3">
                  ...
                  <span className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider pl-2 animate-pulse">Searching vectors...</span>
                </div>
              </div>
            )}
      ...
      <div className="p-4 border-t border-zinc-800/60 bg-zinc-900/10 shrink-0">
        ...
        <div className="max-w-3xl mx-auto text-[9px] text-zinc-600 text-center mt-1.5 font-medium">
          Press Enter to query. Use Shift + Enter for line breaks.
        </div>
      </div>
    </div>
  );
